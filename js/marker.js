Marker = function(app, id, latlng, name, color) {
    this.app = app;
    this.id = id;
    this.name = name;
    this.color = app.style.normalize_color(color);

    this.obj = L.marker(latlng, {draggable: true, icon: app.style.icon[this.color]}).addTo(app.map.map);
    ...
    display.append(display_title).append(content).append(display_buttons);

    var edit = $('<div class="edit hidden">');
    var edit_title = $('<input type="text" class="title" placeholder="Marker Title">');
    ...
    this.obj.on('drag', function(event) { self.handleMoved(false); });
    this.obj.on('dragend', function(event) { self.handleMoved(true); });
};

Marker.prototype = {
    store: function() {
        this.app.storage.write('marker.' + this.id, {
            lat: this.obj.getLatLng().lat,
            lng: this.obj.getLatLng().lng,
            name: this.name,
            color: this.color
        });
    },
    ...
    view: function() {
        this.app.map.map.panTo(this.obj.getLatLng(), {animate: true});
    },
    ...
    remove: function() {
        ...
        for (var i = this.app.marker_list.length - 1; i >= 0; i--) {
            if (this.app.marker_list[i] === this.id) {
                this.app.marker_list.splice(i, 1);
            }
        }
        this.app.storage.write('markers', this.app.marker_list);

        delete this.app.marker_hash[this.id];
    },

    handleMoved: function(store) {
        $('#' + this.id + ' .coords').text(this.obj.getLatLng().toString());
        if (store) {
            this.store();
        }
    },
    ...
    accept_edit: function() {
        ...
        this.obj.setIcon(this.app.style.icon[this.color]);
        this.store();
